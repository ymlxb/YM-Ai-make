 ...
 * - "modification" → 修改请求图 (modification.graph.ts)
 *
 * 职责：
...
import { buildModificationAgent } from "./modification.graph.js";
 ...
 *   - "modification": 基于已有代码的增量修改流程
 */
export function buildAgent(
  mode: "traditional" | "figma" | "modification" = "traditional",
) {
  if (mode === "figma") {
  ...
  if (mode === "modification") {
    console.log("✏️  [MainGraph] 构建修改请求图");
    return buildModificationAgent();
  }
